...
- **Credentials**: a Desktop OAuth client the user supplies in Settings, stored in the Keychain (`clientID` / `clientSecret` accounts) and read back as an ID/secret **pair** by `GoogleCalendarAuth.credentials`. Nothing is embedded in the bundle and nothing lives in the repo: GitHub Releases are public, so a baked-in `client_secret` would be a published one, and the Keychain additionally outlives the bundle an update replaces. No client → `.unavailable`, and Settings offers a file picker and a paste sheet rather than a dead end. Saving or removing a client tears the session down first (`disconnect()` revokes while the client that minted the tokens is still active) and clears tokens, since tokens from another client resurface as an opaque `invalid_grant`. `parseClientJSON` accepts Google's nested `{"installed":…}` / `{"web":…}` download and a flat object, trims, and rejects blank IDs and `YOUR_` placeholders; `CalendarSelfChecks` covers each case.
