...
- **VoiceOver**: every icon-only control has a real `.accessibilityLabel` (or is built from `Label(_:systemImage:)` + `.labelStyle(.iconOnly)`, which gets one for free — see `BackToWelcomeToolbar`'s `button`). `.help()` alone does not provide this — it sets the AX *help* attribute, not the AX *label*, so VoiceOver announced these as unnamed before. Decorative icons (waveform bars, theme-card preview dots) are `.accessibilityHidden(true)` instead. A `.labelsHidden()` control (segmented pickers, sliders) gets an explicit `.accessibilityLabel` since it has no visible one to fall back to. The note-date control is now `ThemedCalendarView`: the month chevrons and each day cell carry their own labels (`"Previous month"` / `"Next month"` / `date.formatted(date: .complete)`), and the calendar itself is labeled `"Note date"`.
...
The card is still shadowless. `HoverLift` is a shadow on *interactive targets*, not on the popup surface. The two shadows have different jobs: `FloatingChrome`'s neutral `.black.opacity(0.15), radius 10, y 3` (`SharedViews.swift:205`) models a panel floating over content; `ThemePalette.hoverShadowColor`'s accent tint at radius 8, y 2 models a target lifting under the pointer. Not to be unified — same spirit as the existing "do not unify the two [chromes]" rule. It lifts the thing the pointer is actually on, never the container around it: `ThemedCalendarView`'s day cells and month arrows and `RecordingActionsPanel`'s rows, not the pill that opens them. A pill that lifts on hover and then opens a popup whose contents *also* lift reads as two competing affordances, and the pill's own 10% accent wash is already cue enough for a chip that size — so the pills stay flat and the lift is reserved for the targets inside a popup. `HoverLift` needs a filled shape underneath — on bare text the shadow traces glyphs, which is why the rows gained a 10%-accent rounded-rect background and the day cells a 10%-accent circle. That circle is also why the today ring now draws as a separate overlay rather than in an `else if`: a hovered today cell needs both. The month arrows take the day cell's `cellSize` circle for the same reason twice over — the bare chevron glyph was a few points wide, so the wash that gives the shadow its shape also fixes a hit target that was much smaller than everything around it.
...
- **Credentials**: a Desktop OAuth client the user supplies in Settings. The **client ID is not a secret** — it is visible in the consent URL — so it lives in UserDefaults (`googleCalendarClientID`); only the secret is keychained. This split is load-bearing: Dosa is ad-hoc signed, so each rebuild changes the code signature a keychain item's ACL was bound to and macOS re-prompts for access, and `clientID` / `hasCredentials` are re-read on every SwiftUI render of the section. `GoogleCalendarKeychain` additionally keeps a write-through in-memory cache (nil results included, so a denied prompt is not a prompt loop), making each account cost at most one prompt per launch; this process is the only writer, so cached values cannot go stale. `credentials` (ID + secret) touches the keychain and is called only by `authorize()` and `refreshAccessToken()`. Nothing is embedded in the bundle and nothing lives in the repo: Releases are public, so a baked-in `client_secret` would be a published one. No client → `.unavailable`, and Settings offers a file picker and a paste sheet rather than a dead end. Saving or removing a client tears the session down first (`disconnect()` revokes while the client that minted the tokens is still active) and clears tokens, since tokens from another client resurface as an opaque `invalid_grant`. `parseClientJSON` accepts Google's nested `{"installed":…}` / `{"web":…}` download and a flat object, trims, and rejects blank IDs and `YOUR_` placeholders. A one-shot migration moves a client ID left in the keychain by an earlier build. `CalendarSelfChecks` covers the parsing and asserts `clientID` stays off the keychain.
