...
Sources/Dosa/            DosaKit library (app + tests)
  ...
  AppSettings.swift      All UserDefaults keys, default prompts, verbosity, appearance, font
  Theme.swift            Preset palettes + accent override + styleFingerprint
  Typography.swift       App-wide font catalog, semantic roles, SwiftUI/AppKit resolution
  ...
  GoogleCalendar/
    GoogleCalendarAuth.swift     Desktop OAuth + PKCE + UserDefaults tokens
    ...
    GoogleCalendarKeychainPurge.swift  One-shot cleanup of the old keychain items
  Views/
...
- **Not themeable by design**: destructive red, the floating overlays' translucent chrome (floating bar / toasts — see `FloatingChrome` in §9c), the titlebar row's glass pills (their *glyphs* can be tinted — the back arrow is — but the pills themselves are the system's), sidebar material. Body text color used to be on this list too (plain system label colors) — accessibility overrides that: see §8b, whose AA floor `Theme.secondaryTextColor` / `Theme.tertiaryTextColor` now set instead.
...
WCAG AA (4.5:1 text, 3:1 non-text) is the floor everywhere in the app, across all 5 presets × 6 accent overrides × both appearances (60 combinations) — not eyeballed, but asserted by `ContrastSelfChecks.run()` (`ContrastSelfChecks.swift`, wired into `DosaCalendarChecks`).

- **Text color tokens** (`Theme.swift`): `Theme.secondaryTextColor` / `Theme.tertiaryTextColor` replace bare `.secondary` / `.tertiary` on text — macOS's `secondaryLabelColor` measures 3.98:1 in light mode (below AA) and `tertiaryLabelColor` ~1.9:1. Fixed values (not per-theme), solved against the tightest `cardFill` across all presets in each mode, and bumped further (8:1 / 7:1) under System Settings ▸ Accessibility ▸ Display ▸ **Increase Contrast**. `ThemePalette.accentText` / `.dangerText` / `.warningText` / `.successText` are per-palette, computed rather than hand-picked: `NSColor.ensuringContrast(_:against:)` darkens or lightens a decorative color (accent, `.systemRed/.systemOrange/.systemGreen`) just enough to clear AA against `cardFill`, preserving hue. `ThemePalette.onAccent` picks white or black for a glyph on a filled accent circle/capsule, via a luminance threshold (`Theme.onAccentLuminanceThreshold`) rather than a hardcoded per-theme guess.
...
- **Resize Text** (WCAG 1.4.4): see the Type scale & Text Size bullet in §8 — a Settings ▸ Theme control, not a fixed scale.
- **Enforcement**: `Scripts/check-accessibility.sh`, run from `build.sh` right after `check-typography.sh`, fails the build on a bare `.foregroundStyle(.secondary/.tertiary)` on text (escape hatch: `// system-color: <reason>`), `.opacity(` applied straight to a `Text(` (escape hatch: `// contrast-ok: <reason>`), and an icon-only `Button`/`Menu` label with no accessible name found nearby (escape hatch: `// a11y-ok: <reason>`) — same grep/awk-level philosophy as the other two check scripts, so the same caveat applies: it catches the pattern appearing at all, not a subtle misuse of it.
...
Storage: **UserDefaults** (`notionAccessToken/RefreshToken/TokenExpiry/ClientId/TokenEndpoint/Workspace…`) — consistent with the LLM provider API keys and, since the keychain came out, with Google Calendar too (§10.5). `validAccessToken()` auto-refreshes within 60 s of expiry.
...
- **Auth** (`GoogleCalendarAuth`): installed-app authorization-code + PKCE, loopback ports 53690–53693, `access_type=offline` + `prompt=consent`, UserDefaults for access/refresh/expiry. Scopes are `calendar.calendarlist.readonly` and `calendar.events.readonly` only. Disconnect best-effort revokes the token.
- **Credentials**: a Desktop OAuth client the user supplies in Settings. Client ID, client secret, access token, refresh token and expiry all live in UserDefaults (`googleCalendarClientID` / `ClientSecret` / `AccessToken` / `RefreshToken` / `TokenExpiry`), consistent with the Notion tokens (§10.4) and the provider API keys. **Nothing is in the Keychain, and nothing should go back in.** It held the tokens until Dosa's ad-hoc signing made it unusable: a generic-password item's ACL is bound to the creating code's signature, an ad-hoc signature's designated requirement is its cdhash, and every `./build.sh` mints a new one — so "Always Allow" was void by the next build and each launch reopened up to four prompts. Writes prompted too (`SecItemDelete`-then-add on a foreign ACL), and hourly token refresh meant they kept coming. The workarounds it forced — keeping `clientID` out of the Keychain because Settings re-reads it on every render, plus a write-through nil-caching memo to cap the prompts at one per item per launch — are all gone. `GoogleCalendarKeychainPurge.runOnce()` fires from `GoogleCalendarManager.init` and `SecItemDelete`s the five old accounts under service `com.dosa.meetingnotes.google-calendar`; it never *reads* one, since only a read decrypts and only a decrypt prompts, and it sets its done-flag before deleting so even a denied dialog cannot recur. That file is disposable once installs have turned over. Nothing is embedded in the bundle and nothing lives in the repo: Releases are public, so a baked-in `client_secret` would be a published one. No client → `.unavailable`, and Settings offers a file picker and a paste sheet rather than a dead end. Saving or removing a client tears the session down first (`disconnect()` revokes while the client that minted the tokens is still active) and clears tokens, since tokens from another client resurface as an opaque `invalid_grant`. `parseClientJSON` accepts Google's nested `{"installed":…}` / `{"web":…}` download and a flat object, trims, and rejects blank IDs and `YOUR_` placeholders. `CalendarSelfChecks` covers the parsing and round-trips a whole client through `saveCredentials`/`credentials`/`clearCredentials` to assert credentials stay on UserDefaults.
...
11. **Tokens/API keys in UserDefaults** — fine for a personal app. Not a Keychain upgrade path while Dosa is ad-hoc signed: item ACLs bind to the code signature, which changes on every build, so the Keychain re-prompts forever — see §10.5. Signing with a stable identity would have to come first, and CI releases are ad-hoc too.
...
15. Accessibility (§8b): `swift run DosaCalendarChecks` passes `ContrastSelfChecks` (all 60 preset × accent-override × appearance combinations). Settings ▸ Theme ▸ Text Size cycles Small→Larger with every surface reflowing live and no clipping (sidebar rows, the 600×660 Settings sheet, the floating bar). System Settings ▸ Accessibility ▸ Display: Increase Contrast visibly deepens muted text; Reduce Transparency makes the floating bar/toasts opaque; Reduce Motion keeps the recording ellipsis fully lit. VoiceOver (⌘F5): every icon-only control in the sidebar toolbar, editor header, floating bar, and Settings announces a name; Tab reaches the previously tap-gesture-only disclosures (sidebar folders/Deleted Notes, Settings prompt groups, the error dialog's technical details) and Space activates them.
