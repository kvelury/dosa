...
- **Color-only signaling** (WCAG 1.4.1): Dosa's additions in the diff view used to be distinguished from manual notes by color alone. `MarkdownStyler.applyDiffColors` (and `DiffEngine.attributedDiff`, its unused-but-kept-in-sync preview counterpart) now also underline them; the "Dosa additions" legend label in `NoteEditorView` matches.
